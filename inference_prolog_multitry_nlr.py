...
def main():    
    parser = argparse.ArgumentParser(description='This script evaluates GPT4 or GPT3.5 Turbo on a subset of the NLR dataset, using our neurosymbolic Prolog augmented approach.')
    ...
    log_file = f"{model}_multiTry_{type}_prolog.txt"
    repeat_max = 50
    ...
    for id, problem in problems.items():
        ...
        if repeated <= repeat_max and prolog_solution and float(prolog_solution) == problem["answer"]:
            count_correct += 1
            if print_progress:
    ...
    print(str(count_correct) + " out of " + str(num_problmes) + " is correct.")
